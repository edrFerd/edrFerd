...
namespace cakeslice
{
	...
	public class OutlineEffect : MonoBehaviour
	{
		...
		public void ClearAllOutlines()
		{
			foreach (var outline in outlines.ToList()) // 遍历集合的副本
			{
				if (outline != null)
				{
					Destroy(outline.gameObject.GetComponent<Outline>()); // 销毁 Outline 组件
				}
			}
			outlines.Clear(); // 清空原集合
		}

		public Outline AddOutline(GameObject go)
		{
